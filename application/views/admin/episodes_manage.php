    ...
    <div class="col-md-6">
      ...
      <div class="panel panel-border panel-primary">
        <div class="panel-heading">
          <h3 class="panel-title"><?php echo trans('add_Product') ?></h3>
        </div>
        <div class="panel-body">
        <ul class="nav nav-tabs" role="tablist">
          <li class="nav-item">
            <a class="nav-link active" data-toggle="tab" href="#tabs-1" role="tab"><?php echo trans('add_Product');?></a>
          </li>
          <li class="nav-item">
          ...
          <div class="panel panel-border panel-primary">
            ...
            <div class="panel-body">
              ...
              <table class="table table-bordered" id="Product-list">
                ...
                <tbody>
                  <?php 
                    $episodes = $this->common_model->get_episodes_by_Products_id_and_season_id($param1,$param2);
                    foreach ($episodes as $Product_file) :
                    ?>
                    <tr id="row_<?php echo $Product_file['episodes_id']; ?>">
                    <td><img src="<?php echo $this->common_model->get_episode_image_url($Product_file['episodes_id'],$param1); ?>" alt="<?php echo $Product_file['episodes_name']; ?>" width="80"></td>
                      <td><strong><?php echo $Product_file['episodes_name']; ?></strong></td>
                      <td><strong><?php echo $Product_file['file_source']; ?></strong></td>
                      <td>
                          <input type="hidden" name="episodes_id[]" value="<?php echo $Product_file['episodes_id']; ?>">
                          <input type="number" name="order[]" value="<?php echo $Product_file['order']; ?>" class="form-control" style="width:80px" required>
                      </td>
                      <td><?php echo $Product_file['file_url']; ?></a></td>
                      <td>
                        ...
                        <?php else : ?>
                          <?php
                              $subtitles = $this->db->get_where('tvseries_subtitle', array('episodes_id' => $Product_file['episodes_id']))->result_array();
                      ...
                      <td>
                        <div class="btn-group">
                          ...
                          <ul class="dropdown-menu" role="menu">
                            <li><a class="dropdown-item" href="<?php echo base_url('admin/episodes_manage/edit/'.$Product_file['episodes_id']); ?>"><?php echo trans('edit_episode') ?></a></li>
                            ...
                            <?php if ($Product_file['file_source'] != 'youtube' && $Product_file['file_source'] != 'vimeo' && $Product_file['file_source'] != 'embed') : ?>
                              <li><a href="#" class="dropdown-item" data-toggle="modal" data-target="#mymodal" data-id="<?php echo base_url() . 'admin/view_modal/tvseries_subtitle_add/' . $Product_file['Products_id'] . '/' . $Product_file['seasons_id'] . '/' . $Product_file['episodes_id']; ?>" id="menu"><?php echo trans('add_subtitle') ?></a> </li>
                            <?php endif; ?>
                            <li><a class="dropdown-item" title="<?php echo trans('delete'); ?>" href="#" onclick="delete_row('episodes',<?php echo urldecode($Product_file['episodes_id']); ?>)" class="delete"><?php echo trans('delete'); ?></a> </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
