<?php $post  = $this->db->get_where('posts', array('post_id' => $post_id))->row() ?>
<div class="card">
	<div class="row">
		<div class="col-sm-12">
			<form method="post" action="<?= base_url() . 'admin/post_edit/' . $post_id; ?>">
				<div class="form-row modal-body p-4">
					<div class="col-md-12">
						<div class="cv-form-group input-group mb-3 mt-4 px-3">
							<div class="input-group-prepend"><span class="input-group-text bg-white rounded-0 cv-chev left">
								...
								</span></div><input id='title' name="post_title" value='<?= $post->post_title ?>' type="text" class="form-control custom-input" required=""><span class="bar"></span><label class="cv-label left text-capitalize">title</label>
						</div>
					</div>
					<div class="col-md-6 d-none">
						<div class="cv-form-group input-group mb-3 mt-4 px-3">
							<div class="input-group-prepend"><span class="input-group-text bg-white rounded-0 cv-chev left">
								...
								</span></div><input id="slug" name="slug" type="text" class="form-control custom-input" required="" autocomplete="new-password" value='<?= $post->slug ?>'><span class="bar"></span><label class="cv-label left text-capitalize">slug</label>
						</div>
					</div>
					<div class="col-md-12 pt-3">
						<div class="form-group px-3">
							<textarea class="wysihtml5 form-control rounded" name="content" id="content" rows="20"><?= $post->content ?></textarea>
						</div>
					</div>
					<div class="col-md-4">
						<div class="cv-form-group form-group mt-4 px-3">
							<select name='category_id[]' class="form-control js-select2">
								...
								foreach ($categories as $category) : ?>
									<option <?= ($post->category_id == $category['post_category_id']) ? 'selected' : ''; ?> value='<?= $category['post_category_id']; ?>'><?= $category['category']; ?></option>
								<?php endforeach; ?>
							</select>
						</div>
					</div>
					<div class="col-md-5">
						<div class="input-group mb-3 mt-4 px-3 d-flex align-items-center">
							...
							<span class="button b2 mt-0 ml-3 mt-2" id="button-10"><input id='status' type="checkbox" name="status" <?= ($post->status == '0') ? 'checked' : '' ?> class="checkbox">
					...
					<div class="col-md-12">
						<div class="cv-form-group input-group mb-3 mt-4 px-3">
							<div class="input-group-prepend"><span class="input-group-text bg-white rounded-0 cv-chev left">
								...
								</span></div><input name="focus_keyword" value='<?= $post->focus_keyword ?>' type="text" class="form-control custom-input" required="" autocomplete="new-password" value=""><span class="bar"></span><label class="cv-label left text-capitalize">focus keywords separated by commas</label>
						</div>
					</div>
					<div class="col-md-12">
						<div class="input-group mb-3 mt-4 px-3">
							<textarea name="meta_description" class="form-control custom-input px-0"><?= $post->meta_description ?></textarea>
							<span class="bar"></span>
