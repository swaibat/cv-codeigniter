        ...
        <div class="row">
            <div class="col-md-6 col-xs-12">
                <h2 class="block-title text-center"><?php echo trans('send_a_request'); ?><br>
                    <small><?php echo trans('product_not_avilable?_send_us_a_note_using_the_form_below.'); ?></small>
                </h2>

                <div class="sendus">
                    <div class="product-heading m-b-20"> <span><?php echo trans('send_us_request'); ?></span>
                        <div class="disable-bottom-line"></div>
                    </div>
                    <div id="contact-form">
                        <div class="expMessage"></div>
                        <form class="custom-form" id="request-form" method="post" action="<?php echo base_url('send_product_requiest'); ?>">
                            ...
                            <input type="text" name="product_name" id="product_name" class="form-control half-wdth-field pull-right" placeholder="Product Name" >
...
<script>
    $(document).ready(function() {
        $("#request-form").submit(function(e) {
            ...
            if (product_name == '') {
                var hasError = true;
                $("#product_name_error").fadeIn();
                $("#product_name_error").html('<p class="text-danger"><strong>Opps!&nbsp;</strong>Product name must not be blank.</p>');
            }
            if (message == '') {
                var hasError = true;
            ...
            if (hasError != true) {
                $.ajax({                    
                    type: 'POST',
                    url: '<?php echo base_url(); ?>send_product_requiest2',
                    data: {"name" : name,"email" : email,"product_name":product_name,"message" : message},
                    dataType: 'json',
                    beforeSend: function() {
                        $("#error").fadeOut();
